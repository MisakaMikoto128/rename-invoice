...
//!
//! 每个 shell 键都写 MultiSelectModel=Player, 否则 legacy verb 默认 Document,
//! 多选超过 15 个文件时右键菜单里这一项会整个消失.
...
/// 见下方 install() 里写 MultiSelectModel 处的注释.
...
pub fn install(_summary: bool, _xlsx: bool) -> Result<()> {
    ...
    // 图标: 优先用同目录的 icon.ico (几 KB).
    // 回退到 .exe 自身嵌入的 ICON RESOURCE, 索引 0 (两个 bin 都嵌了一样的图标).
    //
    // 为什么优先 .ico: Explorer 每次构建右键菜单都要打开 Icon 指向的文件抽图标资源.
    // 指向 ~10 MB 的 .exe 会让菜单弹出明显变慢 (图标缓存冷的时候尤其明显),
    // 指向几 KB 的 .ico 就没这个开销.
    let ico = exe_dir.join("icon.ico");
    let icon_path = if ico.exists() {
        ico.to_string_lossy().to_string()
    } else {
        format!("{},0", silent_exe_str)
    };
    ...
    for (path, cmd, desc) in &targets {
        ...
        // 多选上限: legacy verb (shell\<Verb>\command + %1) 不写 MultiSelectModel 时
        // Shell 默认按 Document 处理 = 最多 15 个选中项, 选到第 16 个菜单直接不显示.
        // Player 把 legacy verb 的上限抬到 100.
        // 语义不变: 仍然是"每个文件启动一次进程", 由 silent.rs 的队列 + leader 锁合并.
        // 超过 100 个仍然不显示, 那种情况直接右键所在文件夹.
        shell_key.set_value("MultiSelectModel", &MULTI_SELECT_MODEL.to_string())?;
    ...
    println!("(多选上限 100 个 PDF; 超过就直接右键所在文件夹, 一次处理整个目录)");
