...
#
# 用法:
#   install_context.ps1                  # 交互式: 默认静默, 可选静默+汇总窗口
...
Write-Host ""
if ($WantSummary) {
    Write-Host "(已启用汇总窗口: 处理完后会弹一个 Tk 窗口列出全部结果)" -ForegroundColor Yellow
} else {
    Write-Host "(静默模式: 完全无窗口, 结果写入 rename_invoice.log)" -ForegroundColor Yellow
}
